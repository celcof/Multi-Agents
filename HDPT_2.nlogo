...
; - valuation is a binary indicating how a trader values either his own or his counterpart's property in a dispute over that property
turtles-own [turtle-energy xhere yhere trys property valuation]
...
; also turtles' property status and valuation parameter must be initialized
ask turtles
[
...
; Also, every owner/intruder is assigned a valuation number (binary choice) representing how much they value their/their counterpart's property
...
; a step. If they cannot move they have trys 9 attempts to head elsewhere and find an accessible spot.
; Agents consume one unit of energy by taking a step.
; Two agents can only end up on the same patch if their ownership status mismatch and if their property valuation mismatch.
; Only one can be the owner and only one the intruder on any single patch at any given time. Neither are identical property valuations allowed.
to move
let here-property property
let here-valuation valuation
let status-match False
let valuation-match False
set xhere xcor
set yhere ycor
set trys 1
let maxtrys 50
while [xhere = xcor and yhere = ycor and trys < maxtrys]
[rt random 46 - random 46
if count turtles-at dx dy = 1
  [ask one-of turtles-at dx dy
      [if here-property = property [set status-match True]]
       if here-valuation = valuation [set valuation-match True]]
if count turtles-at dx dy < 2 and status-match = False and valuation-match = False
[fd 1]
set trys trys + 1]
...
[without-interruption
  [
    ...
    [set my-valuation valuation  ;store current agent's valuation in a new variable as both disputants' valuations are needed later on
     if breed = hawks
       [ask other turtles-here
         [if breed = hawks
           [set turtle-energy (turtle-energy + 0.5 * valuation - cost)
              ask myself [set turtle-energy (turtle-energy + 0.5 * my-valuation - cost)]]    ; asks the turtle who did the last asking
